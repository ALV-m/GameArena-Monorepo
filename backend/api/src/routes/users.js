  ...
  try {
    ...
    const result = await pool.query(
      `SELECT id, username, avatar_url, country, rank_tier, elo_rating, total_wins, total_losses, total_matches
       FROM ga_users WHERE is_banned = FALSE
  ...
  try {
    const result = await pool.query(
      `SELECT id, username, avatar_url, country, rank_tier, elo_rating,
       ...
       FROM ga_users WHERE id = $1 AND is_banned = FALSE`,
      [req.params.id]
    );
...
router.put('/profile', authenticate, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE ga_users SET avatar_url = COALESCE($1, avatar_url), country = COALESCE($2, country), updated_at = NOW()
