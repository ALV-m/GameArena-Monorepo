...
router.get('/dashboard', async (req, res) => {
  try {
    const [users, tournaments, matches, transactions, disputes, revenue] = await Promise.all([
      ...
      pool.query('SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status = \'in_progress\') as active, COUNT(*) FILTER (WHERE status = \'completed\') as completed FROM tournaments'),
      pool.query('SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status = \'completed\') as completed, COUNT(*) FILTER (WHERE status = \'disputed\') as disputed FROM matches'),
      pool.query('SELECT SUM(amount) as total_deposited, SUM(amount) as total_withdrawn FROM transactions WHERE status = \'completed\''),
      pool.query('SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status = \'open\') as pending FROM disputes'),
      pool.query("SELECT SUM(amount) as platform_fees FROM transactions WHERE type IN ('tournament_entry', 'challenge_entry') AND status = 'completed'"),
    ]);
...
router.get('/tournaments', async (req, res) => {
  try {
    ...
    let query = `SELECT t.*, u.username as creator_name FROM tournaments t JOIN users u ON t.creator_id = u.id WHERE 1=1`;
    const params = [];
    let p = 0;
...
router.get('/transactions', async (req, res) => {
  try {
    ...
    let query = `SELECT t.*, u.username FROM transactions t JOIN wallets w ON t.wallet_id = w.id JOIN users u ON w.user_id = u.id WHERE 1=1`;
    const params = [];
    let p = 0;
...
router.get('/disputes', async (req, res) => {
  try {
    ...
    let query = `SELECT d.*, u.username as reporter_name, m.player1_id, m.player2_id,
                 ...
                 LEFT JOIN users u2 ON m.player2_id = u2.id WHERE 1=1`;
    const params = [];
    let p = 0;
...
router.put('/disputes/:id/resolve', async (req, res) => {
  ...
  try {
    ...
    await client.query(
      "UPDATE disputes SET status = $1, admin_id = $2, resolution = $3, resolved_at = NOW() WHERE id = $4",
      [status, req.user.id, resolution, req.params.id]
    );

    if (winner_id) {
      const match = await client.query('SELECT * FROM matches WHERE id = $1 FOR UPDATE', [d.match_id]);
      const m = match.rows[0];
      if (m && parseFloat(m.stake_amount) > 0) {
        const loser_id = winner_id === m.player1_id ? m.player2_id : m.player1_id;
        const wallet = await client.query('SELECT * FROM wallets WHERE user_id = $1 FOR UPDATE', [winner_id]);
      ...
      await client.query("UPDATE matches SET winner_id = $1, status = 'completed', completed_at = NOW() WHERE id = $2", [winner_id, d.match_id]);
    }
...
router.get('/payments', async (req, res) => {
  try {
    ...
    let query = `SELECT p.*, u.username FROM payments p JOIN users u ON p.user_id = u.id WHERE 1=1`;
    const params = [];
    let p = 0;
