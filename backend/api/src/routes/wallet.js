...
router.get('/', authenticate, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT * FROM ga_wallets WHERE user_id = $1`,
      [req.user.id]
    );
...
router.get('/transactions', authenticate, async (req, res) => {
  try {
    ...
    const wallet = await pool.query('SELECT id FROM ga_wallets WHERE user_id = $1', [req.user.id]);
    if (wallet.rows.length === 0) return res.status(404).json({ error: 'Wallet not found' });

    const result = await pool.query(
      `SELECT * FROM ga_transactions WHERE wallet_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
...
router.post('/credit', authenticate, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `UPDATE ga_wallets SET balance = balance + $1,
